...
class GPTAnswer:
    ...
    def __init__(self):
        ...
        config_path = os.path.join(os.path.dirname(__file__), 'config', 'config_local_llm.yaml')
        ...
        # model_checkpoint_Llama = '/home2/bazaarz/desktop/Llama3/Llama/Llama'
        # model_checkpoint_Phi3_4k = '/home2/bazaarz/desktop/Llama3/Llama/Phi3-4k'
        # model_checkpoint_Phi3_128k = '/home2/bazaarz/desktop/Llama3/Llama/Phi3-128k'
        model_checkpoint = "maywell/Llama-3-Synatra-11B-v1"
        # self.llm = ChatOllama(model=model_checkpoint, temperature=0,streaming=True, callbacks=[StreamingStdOutCallbackHandler()])
        self.llm = HuggingFacePipeline.from_model_id(model_id="maywell/Llama-3-Synatra-11B-v1",task="text-generation",
                                                     pipeline_kwargs={"torch_dtype":torch.bfloat16,"return_full_text": False,"temperature": 1e-20,"max_new_tokens": 2024},
                                                     device_map = 'cuda',callbacks=[StreamingStdOutCallbackHandler()])
    ...
    def get_keyword(self, query, language, output_format, profile):
        ...
        gpt_answer = self.llm.invoke([HumanMessage(content=summary_prompt)])
        gpt_answer = gpt_answer.split("<|end_header_id|>")[-1]
        return gpt_answer
    def get_answer(self, query, relevant_docs, language, output_format, profile, ids):
        # Create an instance of ChatOpenAI and generate an answer
        # summary_prompt = prompt_template.format(context_str=relevant_docs, language=language, query=query, format=output_format, profile=profile, information = ids )
        # gpt_answer = self.talk.predict(input=summary_prompt)
        
        template = self.config["template"]
        prompt_template = PromptTemplate(
            input_variables=["profile", "context_str", "language", "query", "format", "information"],
            template=template
        )
